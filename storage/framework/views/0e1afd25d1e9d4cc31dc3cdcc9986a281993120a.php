    ...
    <div class="cl-inner">
        <div class="cl-container">
                ...
                <div id="chat-body" class="cl-chat-box">
                    <div class="cl-noti">
                        <dl>
                            <dt>NOTICE</dt>
                            <dd>
                                <?php echo e(__('chat notice1')); ?>

                                <?php echo e(__('chat notice2')); ?>

                            </dd>
                        </dl>
                        <dl>
                            <dt>NOTICE</dt>
                            <dd> <?php echo e(__('chat notice3')); ?></dd>
                            <dd> <?php echo e(__('chat notice4')); ?></dd>
                            <dd> <?php echo e(__('chat notice5')); ?></dd>
                            <dd> <?php echo e(__('chat notice6')); ?></dd>
                        </dl>
                    </div>
                </div>
                ...
                <div class="chat-text-area">
                    <form id="chat-form">
                        ...
                        <label id="attach-file" for="file-input" class="add-photo" title="사진 첨부">
                            <img src="<?php echo e(asset('/public/pub')); ?>/img/chat-add-photo.png" alt="사진 첨부"
                                style="width: 22px;">
                        </label>
                        ...
                        <button type="submit" class="send-msg" title="메세지 보내기">
                            <img src="<?php echo e(asset('/public/pub')); ?>/img/chat-send@2x.png">
                        </button>
                    </form>
                </div>
            </div>
            ...
            <button id="close-chat" class="btn-cl-close" title="채팅창 닫기"><img
                    src="<?php echo e(asset('/public/pub')); ?>/img/close-12@2x.png"></button>
        </div>
    </div>
</div>
<!-- chat-layer -->
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        <?php if(isset($currentUserId)): ?>
            currentUserId = <?php echo e($currentUserId); ?>;
        <?php endif; ?>
        <?php if(isset($receiverId)): ?>
            window.receiverId = <?php echo e($receiverId); ?>;
        <?php endif; ?>
        ...
        function bindPusherEvents() {
            ...
            window.pusherInstance.bindEvent('new.notification', (data) => {
                ...
                if(data.userNotification.type === 'TRADE_CANCELLED'){ // 취소로 변경시 
                    ...
                    alert(data.userNotification.message.message);
                    window.location.href = '<?php echo e(route('user.wallet.index')); ?>';
                }
            ...
            function addNewNotification(data) {
                ...
                newNotification.innerHTML = `
                    <div class="thumb">
                        <img src="${data.user_image || '<?php echo e(auth()->user()->userImage); ?>'}" alt="user">
                    </div>
                    <div class="content">
            ...
            function updateNotificationList() {
                fetch('<?php echo e(route('user.messages.getNotifications')); ?>') // 이 route는 서버에서 구현해야 합니다
                                ...
                                li.addEventListener('click', function() {
                                    if (item?.type === 'TRADE_STARTED') {
                                        window.location.href =
                                            `<?php echo e(route('user.dashboard')); ?>/${item.order_id}`;
                                    } else if (item?.type === 'USDT_RECEIVED' || item?.type === 'USDT_SENT') {
                                        window.location.href =
                                            `<?php echo e(route('user.wallet.index')); ?>`;
                                    }
                                });
                            }
        ...
        function startChat(orderId, senderId) {
            ...
            function fetchReceiverName(chatId) {
                return fetch(`<?php echo e(setRoute('user.messages.getReceiverName')); ?>?receiver_id=${chatId}`)
                    .then(response => response.json())
        ...
        function loadMessages(chatId) {
            ...
            fetch(`<?php echo e(setRoute('user.messages.getMessages')); ?>?chat_id=${chatId}`)
                .then(response => response.json())
                .then(data => {
                    // console.log(data);
                    chatBody.innerHTML = `
                    ...
                    /* 이전 버전
                           <div class="cl-noti">
                            <dl>
                                <dt>NOTICE</dt>
                                <dd style="color: blue;">
                                    <?php echo nl2br(__('chat notice1')); ?>
                            ...
                            <dl>
                                <dt>NOTICE</dt>
                                <dd style="color: blue;"> <?php echo e(__('chat notice3')); ?></dd>
                                <dd style="color: blue;"> <?php echo e(__('chat notice4')); ?></dd>
                                <dd style="color: blue;"> <?php echo e(__('chat notice5')); ?></dd>
                                <dd style="color: blue;"> <?php echo e(__('chat notice6')); ?></dd>
                                <dd style="color: blue;"> <?php echo e(__('chat notice7')); ?></dd>
                            </dl>
                        </div>
                    */
                ...
                .catch(error => {
                    ...
                    alert(`<?php echo e(__('메시지를 불러오는데 실패했습니다. 페이지를 새로고침해주세요.')); ?>`);
                });
        }
        ...
        function sendMessage(message, retryCount = 3) {
            ...
            if (currentChatId) {
                ...
                fetch('<?php echo e(setRoute('user.messages.sendMessage')); ?>', {
                        method: 'POST',
                        headers: {
                    ...
                    .catch(error => {
                        ...
                        if (retryCount > 0) {
                            ...
                            alert(`<?php echo e(__('메시지 전송에 실패했습니다. 다시 시도해주세요.')); ?>`);
                        }
                    });
            }
        }
        ...
        function uploadFile(file) {
            ...
            if (file.size > maxSize) {
                alert(`<?php echo e(__('파일 크기가 10MB를 초과합니다. 더 작은 파일을 선택해주세요.')); ?>`);
                return;
            }
            ...
            xhr.open('POST', '<?php echo e(route('user.messages.uploadFile')); ?>', true);
            ...
            xhr.onload = function() {
                ...
                if (xhr.status === 200) {
                    ...
                    if (response.success) {
                        ...
                        alert(`<?php echo e(__('파일 업로드에 실패했습니다. 다시 시도해 주세요.')); ?>`);
                    }
                } else {
            ...
            xhr.onerror = function() {
                ...
                alert(`<?php echo e(__('파일 업로드 중 오류가 발생했습니다. 다시 시도해 주세요.')); ?>`);
            };
...
<?php /**PATH /var/www/html/resources/views/user/layouts/chat.blade.php ENDPATH**/ ?>