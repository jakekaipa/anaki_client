                            ...
                            <div class="buttons-and-search-container" style='margin-top:1%'>
                                <div class="buttons-container">
                                    <select class="form-select" name='transactionType' aria-label="Select Option 1" >
                                        <option value=''     {{ ($transactionType ==='')? 'selected':'' }} >거래타입</option> 
                                        <option value="sell" {{ ($transactionType ==='sell')? 'selected':'' }}>판매</option>
                                        <option value="buy"  {{ ($transactionType ==='buy')? 'selected':'' }}>구매</option>
                                    </select>

                                    <select class="form-select" name='state' aria-label="Select Option 2">
                                        <option value=''               {{ ($state =='')? 'selected':'' }} >거래상태</option>
                                        <option value="open"           {{ ($state =='open')? 'selected':'' }}>거래시작</option>
                                        <option value="send"           {{ ($state =='send')? 'selected':'' }}>입금증빙전송</option>
                                        <option value="cancel"         {{ ($state =='cancel')? 'selected':'' }}>거래취소</option>
                                        <option value="dispute"        {{ ($state =='dispute')? 'selected':'' }}>분쟁</option>
                                        <option value="dispute-solved" {{ ($state =='dispute-solved')? 'selected':'' }}>분쟁해결</option>
                                        <option value="done"           {{ ($state =='done')? 'selected':'' }}>거래완료</option>
                                    </select>

                                    <div class="date-select-container" style="display: flex; align-items: center;">
                                        <label for="startDate" style=" width: 180px;">기간선택:</label>
                                        <input type="date" class="form-control form-control-sm" name="startDate" id="startDate" value={{ $startDate }} style="width: 50%" placeholder="시작날짜">
                                        
                                        <span style="margin: 0 10px;">/</span>

                                        <!-- <label for="endDate" style="width: 100px;">종료날짜: </label> -->
                                        <input type="date" class="form-control form-control-sm" name="endDate" id="endDate" value={{ $endDate }} style="width: 50%;" placeholder="종료날짜">
                                    </div>
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            @foreach($transactionList as $list)
                                <tr style="text-align: center;">
                                    ...
                                    <td style="width:6%">{{ ($list->order_type ==='sell')? '판매':'구매' }}</td>
                                    <td style="width:15%">
                                        {{ ($list->order_type ==='sell')? $list->offer_user->username: $list->client_user->username  }}
                                        ({{ ($list->order_type ==='sell')? $list->offer_user->realname: $list->client_user->realname  }})
                                    </td>
                                    <td style="width:15%">
                                        {{ ($list->order_type ==='sell')? $list->client_user->username : $list->offer_user->username  }}
                                        ({{ ($list->order_type ==='sell')? $list->client_user->realname: $list->offer_user->realname  }})
                                    </td>
                                    <td style="width:8%">{{ $list->tetherAmount }}</td>
                                    <td style="width:11%">{{ $list->fee }}</td>
                                    @php  
                                    if($list->state === 'open'){
                                        $stateText = '거래시작';
                                    ...
                                    @endphp
                                    <td style="width:7%">{{ $stateText }} </td>
                                    <td style="width:11%">{{ $list->created_at }}</td>
                                    <td style="width:13%">
                                    @if($list->state === 'dispute')    
                                        <input type="button" class="btn btn-block btn-primary btn-xs" style='width:50%' onclick="openViewModal('{{ urlSafeEncrypt($list->id) }}')" value="보기">
                                        <input type="button" class="btn btn-block btn-danger btn-xs" style="width:40%;margin-top:0;" onclick="openChatModal('{{ urlSafeEncrypt($list->id) }}')"  value="채팅보기">
                                    @else
                                        <input type="button" class="btn btn-block btn-primary btn-xs" style='width:60%' onclick="openViewModal('{{ urlSafeEncrypt($list->id) }}')" value="보기">
                                    @endif
                                    </td>
                                </tr>
                            @php $count++ @endphp
