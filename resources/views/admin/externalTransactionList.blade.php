        ...
        <div class="container-fluid px-4">
            ...
            <div class="card mb-4">
                <div class="card-header">
                    ...
                    전체 외부거래 건수 :  {{ $externalTransactionCount }}
                </div>
                    ...
                    <form id="submitForm" action="" method="GET">
                        <input type="hidden" name="page" id="pages" value="{{ $page }}">                               
                            ...
                            <div class="buttons-and-search-container" style='margin-top:1%'>
                                ...
                                <select class="form-select" name='transferType' aria-label="Select Option 1" >
                                    <option value = '' {{ ($transferType =='')? 'selected':'' }} >구분</option> 
                                    <option value = 1  {{ ($transferType == 1)? 'selected':'' }}>전송</option>
                                    <option value = 2  {{ ($transferType == 2)? 'selected':'' }}>받음</option>
                                </select>

                                <div class="date-select-container" style="display: flex; align-items: center;">
                                    <label for="startDate" style="width: 180px;">기간선택:</label>
                                    <input type="date" class="form-control form-control-sm" name="startDate" id="startDate" value={{ $startDate }} style="width:70%;" placeholder="시작날짜">
                                    
                                    <span style="margin: 0 10px;">/</span>

                                    <input type="date" class="form-control form-control-sm" name="endDate" id="endDate" value={{ $endDate }} style="width: 70%;" placeholder="종료날짜">
                                </div>
                                ...
                                <div class="searchGroup">                                    
                                    ...
                                    <input type="search" id="searchInput" class="form-control form-control-sm" name="searchKey" autocomplete="off" style="width:60%" placeholder="검색어를 입력하세요" value='{{ $searchKey }}'>
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            @php 
                                $count = ($page-1)*10+1; 
                            @endphp
                            @foreach($externalTransactionList as $list)
                                <tr style="text-align: center;">
                                    <td style="width:3%">{{ $count }}</td>
                                    <td style="width:11%">{{ $list->getUserInfo->username }}({{ $list->getUserInfo->realname }}) </td>
                                    <td style="width:6%">{{ ($list->transfer_type == 1)? '전송':'받음' }}</td>
                                    <td style="width:6%">{{ round($list->amount,3) }}</td>
                                    <td style="width:5%">{{ round($list->network_fee) }}</td>
                                    <td style="width:11%">{{ ($list->from)??$list->to }}</td>
                                    <td style="width:9%">{{ $list->created_at }}</td>
                                </tr>
                            @php $count++ @endphp
                            @endforeach
                            </tbody>
