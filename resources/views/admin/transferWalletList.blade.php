        ...
        <div class="container-fluid px-4">
            ...
            <div class="card mb-4">
                <div class="card-header">
                    ...
                    전체 지갑 간 전송 건수 :  {{ $walletTransferCount }}
                </div>
                    ...
                    <form id="submitForm" action="" method="GET">
                        <input type="hidden" name="page" id="pages" value="{{ $page }}">                               
                            <div class="btn-group-lg" role="group" aria-label="Basic radio toggle button group">
                                ...
                                <input type="button" class="btn btn-primary active" onclick='moveTransferWalletList()' value='지갑간전송' autocomplete="off" checked>
                            </div>
                        ...
                        <table class="table table-bordered table-striped">
                            ...
                            <tbody>
                                @php 
                                    $count = ($page-1)*10+1; 
                                @endphp
                                @foreach($walletTransferList as $list) 
                                    <tr style="text-align: center;">
                                        <td style="width:5%">{{ $count }}</td>
                                        <td style="width:12%">{{ $list->senderUsername }}( {{ $list->senderRealName }} )</td>
                                        <td style="width:11%">{{ $list->receiverUsername }} ( {{ $list->receiverRealName }} )</td>
                                        <td style="width:11%">
                                        @if(floor($list->amount) == $list->amount)
                                            {{ number_format($list->amount, 0) }} 
                                        @else
                                            {{ number_format($list->amount, 5) }}  
                                        @endif
                                        </td>
