    ...
    <tbody>
        @forelse ($currencies ?? [] as $item)
            <tr data-item="{{ $item->editData }}">
                ...
                <td>{{ $item->symbol }}</td>
                <td><span class="{{ $item->type == 'FIAT' ? 'text--info' : 'text--warning' }}">{{ $item->type }}</span> <br> 1 {{ get_default_currency_code($default_currency) }} = {{ get_amount($item->rate,$item->code,10) }}</td>
                <td>
                    @if (!$item->isDefault())
