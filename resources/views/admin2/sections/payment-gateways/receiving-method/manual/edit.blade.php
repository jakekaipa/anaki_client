...
@section('content')
    <form action="{{ setRoute('admin.payment.gateway.update',['receiving-method','manual',$payment_gateway->alias]) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method("PUT")
                        ...
                        <div class="form-group">
                            ...
                            <select class="form--control select2-auto-tokenize currency_name" name="currency_name">
                                ...
                                @foreach ($currencies as $item)
                                   <option value="{{ $item->name }}" {{ $payment_gateway->currency->currency_code == $item->code ? 'selected' : '' }} data-currency-code="{{ $item->code }}" data-currency-symbol="{{ $item->symbol }}">{{ $item->name }}</option>
                                @endforeach
                            </select>
                       </div>
                        <div class="form-group">
