<div class="p-head">
    <div class="p-head__col mb flex-s-c">
        <button class="p-head__menu-btn"><img src="{{ asset('/public/pub') }}/img/p-head-mm.png"></button>
        <p class="p-head-tit">{{ __($page_title) }}</p>
    </div>

    <div class="p-head__col flex-s-c">
        {{-- <div class="sch-inp">
            <form id="searchForm" onsubmit="return handleSearch(event)">
                <div class="sch-inp__box">
                    <button class="sch-inp__sch" title="{{ __('Search') }}"><img
                            src="{{ asset('/public/pub') }}/img/search@2x.png"></button>
                    <input type="text" id="schAuto" class="sch-inp__input" placeholder="{{ __('Search') }}"
                        title="{{ __('Enter search term') }}">
                    <button class="sch-inp__del" onclick="delKey();" title="{{ __('Delete') }}"><img
        ...
        <div class="user-profile">
            <a href="{{ setRoute('user.profile.index') }}" class="user-profile__btn">
                <img src="{{ auth()->user()->userImage }}" alt="profile">
            </a>
        </div>
        {{-- <button id="start-chat-header" class="push-icon" style="display:none"> --}}
    </div>
</div>
...
<script>
    $(document).ready(function() {
        ...
        $('.btn-select-lang').click(function(e) {
            ...
            $.ajax({
                url: '{{ route('user.language.switch') }}',
                type: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    target: selectedLang
        ...
        document.querySelector('.p-head__menu-btn').addEventListener('click', function() {
            ...
            fetch('{{ route('update.side.menu.session') }}', {
                    method: 'POST',
                    headers: {
