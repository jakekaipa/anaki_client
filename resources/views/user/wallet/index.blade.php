...
@section('content')
    <div class="content wallet">
        <h2 class="page-tit">{{ __('내 지갑') }}</h2>

        <div class="container-row card-box-wrap mt-0">
            <div class="card-box wallet-box">
                <strong class="cb-tit">{{ __('자산 (USDT)') }}</strong>
                <p class="wallet-property tether">
                    <strong>$&nbsp{{ number_format($usdtBalance, 2) }} </strong>
                </p>
                <p class="wallet-property-sub">{{ __('Pending') }} : $&nbsp{{ number_format($usdtPending, 2) }}</p>

                <div class="wb-button">
                    <button class="btn-wb" onclick="openPopSheet('sheet-send_usdt')"><span
                            class="send txt">{{ __('보내기') }}</span></button>
                    <button class="btn-wb" onclick="openPopSheet('sheet-receive_usdt')"><span
                            class="recieve txt">{{ __('받기') }}</span></button>
                </div>
            </div>
        </div>
        ...
        <div class="container-row card-box-wrap">
            <div class="card-box">
                <strong class="cb-tit">{{ __('최근 트랜잭션') }}</strong>

                <div class="table-wrap">
                    <table class="wallet-tbl">
                        <caption>{{ __('최근 트랜잭션 게시판') }}</caption>
                        <colgroup>
                            <col class="wt-col1">
                            <col class="wt-col2">
                            <col class="wt-col3">
                        ...
                        <tbody>
                            @forelse ($listData as $item)
                                <tr>
                                        ...
                                        @if ($item->type === __('출금'))
                                            <div class="result r-complete">
                                                @if (!empty($item->txid))
                                                    ...
                                                    ?></p>
                                                @else
                                                    <p class="rc-txt1">{{ __('전송중') }}</p>
                                                @endif
                                            </div>
                                        @elseif ($item->type === __('입금'))
                                        ...
                                        <p class="detail">
                                            @if (!empty($item->txid))
                                                <button type="button" class="name"
                                                    onclick="onViewDetail('{{ $item->txid }}')">
                                                    {{ $item->txid }}
                                                </button>
                                            @endif
                                        </p>
                                    ...
                                    <td>
                                        <div class="value">
                                            <span class="val1">{{ number_format($item->amount, 2) }} USDT</span>
                                            {{-- <span class="val2">
                                                {{ number_format($item->amount * $prices['KRW']) }}
                                                {{ __('KRW') }}</span> --}}
                                        </div>
                                    </td>
                                </tr>
                            @empty
                                <tr>
                                    <td colspan="4" class="text-center"><span
                                            class="text-danger">{{ __('No Records Found') }}</span></td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
    ...
    <div class="pop-sheet" data-pop-sheet="sheet-send_usdt">
        <div class="ps-inner">
            <div class="ps-container">
                <div class="ps-head">
                    <h2 class="ps-tit">{{ __('Tether 보내기') }}</h2>
                </div>

                <div class="ps-content custom-scroll trans">
                    <div class="ws2">
                        <div class="amw-qr">
                            <p class="txt">{{ __('ANAKI는 TRON 네트워크(TRC-20)만을 사용합니다.') }}</p>
                        </div>

                        <div class="send-row">
                            <span class="sr-txt">{{ __('테더(TRC20)보내기') }} </span>
                            
                            <div class="sr-btn">
                                <button class="sr-btn2">{{ __('외부지갑') }}</button>
                                <button class="sr-btn1">{{ __('ANAKI USER') }}</button>
                            </div>
                        </div>
                        ...
                        <div class="i-txt__wrap">
                            <div class="i-txt__box">
                                <input type="text" id="recipientAddress" name="walletAddr" class="i-txt__input"
                                    title="{{ __('받을 사람의 지갑 주소를 입력하세요') }}" placeholder="{{ __('받을 사람의 지갑 주소를 입력하세요') }}">
                        ...
                        <div class="flex-container">
                            ...
                            <button id="qrCodeButton" onclick='qrCodeScan()'>{{ __('QrCode Scan') }}</button>
                        </div>
    ...
    <div class="pop-sheet" data-pop-sheet="sheet-receive_usdt">
        <div class="ps-inner">
            <div class="ps-container">
                <div class="ps-head">
                    <h2 class="ps-tit">{{ __('Tether 받기') }}</h2>
                </div>
                ...
                <div class="ps-content custom-scroll trans">
                    <div class="wr2">
                        <div class="amw-qr">
                            <p class="txt">{{ __('ANAKI는 TRON 네트워크(TRC-20)만을 사용합니다.') }}</p>
                            <div class="qr-img"><img src="{{ asset('/public/pub') }}/img/auth-qr@2x.png"></div>
                        </div>
                        <div class="i-txt__wrap">
                            <label for="wallet1" class="i-txt__label">{{ __('내 TRC-20 주소') }}</label>
                            <div class="i-txt__box">
                                <input type="text" id="wallet1" class="i-txt__input addr-wallet" readonly
                                    title="내 지갑 주소가 입력 되어 있습니다." value="{{ $user->walletAddress }}">
                                <button type="button" class="i-txt__copy btn-addr-wallet" title="입력내용 복사"><img
                                        src="{{ asset('/public/pub') }}/img/copy@2x.png"></button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="ps-process">
                    <button class="ps-process-btn cancel">
                        <span class="txt">{{ __('주소공유') }}</span>
                    </button>
                    <button class="ps-process-btn">
                        <span class="txt">{{ __('클립보드에 복사') }}</span>
                    </button>
                </div>
...
@push('script')
    ...
    <script>
        var krwRate = {{ $prices['KRW'] }};

        const TRON_NETWORK = "{{ config('services.tron.network') }}";
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function copyToClipboard(text, successMessage = `{{ __('텍스트가 클립보드에 복사되었습니다!') }}`) {
                if (navigator.clipboard) {
                    navigator.clipboard.writeText(text).then(function() {
                        alert(successMessage);
                    }).catch(function(err) {
                        console.error('텍스트 복사 실패:', err);
                        alert(`{{ __('복사에 실패했습니다. 수동으로 복사해 주세요.') }}`);
                    });
                } else {
            ...
            $('.btn-addr-wallet').on('click', function() {
                ...
                copyToClipboard(walletAddress, `{{ __('지갑 주소가 클립보드에 복사되었습니다!') }}`);
            });
            ...
            function fallbackShare(address) {
                ...
                try {
                    document.execCommand('copy');
                    alert(`{{ __('지갑 주소가 클립보드에 복사되었습니다. 원하는 앱에 붙여넣기 하세요.') }}`);
                } catch (err) {
                    console.error('주소 복사 실패:', err);
                    alert(`{{ __('주소 복사에 실패했습니다. 수동으로 복사해 주세요.') }}`);
                }
            ...
            const krwRate = {{ $prices['KRW'] }}; // PHP에서 JavaScript로 환율 전달
            ...
            const usdtBalance = "{{ $usdtBalance }}"; // Get the USDT balance from PHP
            ...
            function updateUIForTransferType(isExternal) {
                if (isExternal) { // 외부 
                    ...
                    recipientAddressInput.placeholder = "{{ __('받을 사람의 지갑 주소를 입력하세요') }}";
                    ...
                    recipientAddressInput.placeholder = "{{ __('받을 사람의 이메일 주소를 입력하세요') }}";
            ...
            const twoFactorEnabled = {{ auth()->user()->two_factor_tether_transfer ? 'true' : 'false' }};
            ...
            sendButton.addEventListener('click', function() {
                ...
                if (isNaN(amount) || parseFloat(amount) <= 0) {
                    alert(`{{ __('유효한 금액을 입력해주세요.') }}`);
                    return;
                }
                
                if (!recipientAddress || !amount) {
                    alert(`{{ __('받는 사람의 주소/이메일과 금액을 모두 입력해주세요.') }}`);
                    return;
                }
                ...
                if (isExternalWallet) {                 
                    ...
                    if (!isTronAddress(recipientAddress)) {
                        alert(`{{ __('주소가 정확한지 확인을 하신 후 다시 보내기를 하세요') }}`);
                        return;
                    }
                ...
                const confirmMessage = isExternalWallet ?
                    `${amount} USDT ${recipientAddress} {{ __('주소로 보내시겠습니까?') }}` :
                    `${amount} USDT ${recipientAddress} {{ __('이메일 주소로 보내시겠습니까?') }}`;
            ...
            function sendUSDT() {
                ...
                const apiEndpoint = isExternalWallet ?
                    "{{ setRoute('user.wallet.sendUSDT') }}" : 
                    "{{ setRoute('user.wallet.transferUSDTInternallyWalletToWallet') }}";
