...
<div id="Phase2" class="content offer offer2">
    ...
    <input type='hidden' name='orderType' id='orderTypes' value="{{  $tradeInfo->order_type }}">
        ...
        <div class="flex-container">
            ...
            <div class="card-box border">
                ...
                <div>
                    ...
                    <div class="dt-row">
                        <!-- 최소 금액 입력란 -->
                        <div class="dt-col">
                            <label for="tradeVolMin" class="dt-label">{{ __('Set Transaction Amount') }}</label>
                            <label class="dt-label"style="margin-left: 10px; color: rgb(83,126,238)">{{ __('Min. 15,000원 ~ Max. 제한없음') }}</label>
                            <div class="dt-inp">
                                <input type="text" id="tradeVolMin" class="input" placeholder="{{ __('Minimum Amount') }}" name="tradeVolMin"
                        ...
                        <div class="dt-col">
                            <div class="dt-inp">
                                <input type="text" id="tradeVolMax" class="input" placeholder="{{ __('Maximum Amount') }}" name="tradeVolMax"
                    ...
                    <div class="section" id="MarketPriceGroup">
                        ...
                        <div class="setting-inp money">
                            <input type="text" class="si-inp inp-m" name="offerMargin" value="{{ ($tradeInfo->priceType == 0)? $tradeInfo->offerMargin : 5 }}"
                    ...
                    <div class="section" id="FixedPriceGroup">
                        ...
                        <div class="setting-inp money">
                            <input type="number" class="si-inp inp-m" value="{{ ($tradeInfo->priceType == 1)? intval($tradeInfo->fixedPrice) : $prices['KRW'] }}" 
            ...
            <div class="offer3-inner">
                ...
                <div class="section">
                    ...
                    <div class="textarea">
                        <textarea name="offerCondition" id="offerCondition" class="w-full" style='height:25%'>{{ $tradeInfo->offerCondition }}</textarea>
                    </div>
                </div>

                <div class="section">
                    <button type="button" onclick="copyLink('{{ $tradeOfferId }}')" class="btn-submit w-full">{{ __('링크주소 복사') }}</button>
                </div>      
...
<script>
    ...
    function onSubmit(){
        ...
        formData.forEach((value, key) => {
          ...
          if(orderTypes === 'sell'){
            const availableUsdt = {{ $availableUsdt }}; 
            if(jsonData.priceType == 1 ) { // 고정가
                if(parseFloat(jsonData.tradeVolMax) > ( availableUsdt * jsonData.fixedPrice)){
                    alert("{{ __('User does not have sufficient balance.') }}")
                    return false;
                }
            } else { // 시장가 
                if(parseFloat(jsonData.tradeVolMax) > ( availableUsdt * ( ( {{ $prices['KRW'] }} * (100 + jsonData.offerMargin) ) / 100 ) )) {
                    alert("{{ __('User does not have sufficient balance.') }}")
                    return false;
                }
            }
    ...
    // 뒤로가기
    function goBack(){
        var tradeId = "{{ $tradeOfferId }}";
        if('{{ $tradeInfo->order_type }}' === 'buy'){
