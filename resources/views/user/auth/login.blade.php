            ...
            <div class="login__head" style="position: relative;">
                <div class="language-selector" style="position: absolute; top: -40px; right: 0;">
                    <select class="lang-select" style="padding: 5px; border-radius: 5px; border: 1px solid #ccc;" onchange=changeLang(this.value)>
                        ...
                        <option value="ko" {{ ($selectLang === 'ko')? 'selected':'' }}>한국어</option>
                    </select>
                </div> 
                <h1 class="logo"><img src="{{ asset('/public/pub') }}/img/logo-basics@2x.png" alt="anaki"></h1>
            </div>
                ...
                <form action="{{ setRoute('user.login.submit') }}" method="POST">
                    ...
                    <div class="i-txt email w-full">
                        <div class="i-txt__wrap">
                            <input type="text" class="i-txt__input" name="email" title="이메일 입력하세요" placeholder="{{ __('이메일') }}">
                    ...
                    <div class="login-find flex-c-s">
                        <a href="{{ setRoute('user.password.forgot') }}" class="login-find-link">{{ __('Find Password') }}</a>
                    </div>
                ...
                <div class="esay-login">
                    <span class="txt">{{ __('간편 로그인') }}</span>
                </div>

                <div class="ss-link">
                    {{-- <a href="#" class="ss-link-a" title="카카오 로그인" onclick="oauthLogin('kakao')"><img
                            src="{{ asset('/public/pub') }}/img/kakao-talk@2x.png"></a> --}}
                    <a href="#" class="ss-link-a" title="구글 로그인" onclick="oauthLogin('google')"><img
                            src="{{ asset('/public/pub') }}/img/google@2x.png"></a>
                    {{-- <a href="#" class="ss-link-a" title="애플 로그인" onclick="oauthLogin('apple')"><img
                            src="{{ asset('/public/pub') }}/img/apple@2x.png"></a>
                </div> --}}
            </div>

            <div class="go-reg flex-c-s">
                <p class="info-txt">{{ __('아직 회원이 아니신가요?') }} <a href="{{ setRoute('user.register') }}" class="point-color tdu">{{ __('Sign Up') }}</a>
                </p>
            </div>
        </div>
    </div>
...
@push('script')
    ...
    <script>
        ...
        function oauthLogin(provider) {
            var urlSessionData = @json(Session::get('url'));
