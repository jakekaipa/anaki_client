...
@section('content')
    <div class="full-container flex-c-c f8f8f8">
        <div class="join2">
            <form method="POST" action="{{ setRoute('user.register.submit') }}" onsubmit="return submitCheck();" >
                @csrf
                <div class="form-box">
                    <div class="join-head">
                        <h1 class="join-tit">{{ __('sign_up') }}</h1>
                    </div>
                    <!-- 레퍼럴 코드 -->
                    <input type='hidden' name='referralCode' value='{{ $referralCode}}'>

                    <div class="i-txt uname w-full">
                        <label for="first_name" class="i-txt__label">{{ __('First Name') }}</label>
                        <div class="i-txt__wrap">
                            <input type="text" id="first_name" name="first_name" class="i-txt__input" 
                                title="{{ __('please_enter_first_name') }}"
                                placeholder="{{ __('please_enter_first_name') }}"
                                value="{{ old('first_name') }}"
                                maxlength="9">
                            <button type="button" class="i-txt__del" title="{{ __('delete_input') }}"><img
                                    src="{{ asset('/public/pub') }}/img/elimination@2x.png"></button>
                        </div>
                    </div>

                    <div class="i-txt uname w-full">
                        <label for="last_name" class="i-txt__label">{{ __('Last Name') }}</label>
                        <div class="i-txt__wrap">
                            <input type="text" id="last_name" name="last_name" class="i-txt__input"
                                title="{{ __('please_enter_last_name') }}" placeholder="{{ __('please_enter_last_name') }}"
                                 value="{{ old('last_name') }}"
                                 maxlength="9">
                            <button type="button" class="i-txt__del" title="{{ __('delete_input') }}"><img
                                    src="{{ asset('/public/pub') }}/img/elimination@2x.png"></button>
                    ...
                    <div class="i-txt w-full">
                        <label for="email" class="i-txt__label">{{ __('email') }}</label>
                        <div class="i-txt__wrap" style='display: flex;'>
                            <input type="text" id="email" name="email" class="i-txt__input" style='margin-right: 8px;' title="{{ __('please_enter_email') }}" placeholder="{{ __('email') }}" >
                            <button type='button' id='verificationCodeButton' class='btn-solid large' style='width:30%' onclick='sendEmailVerificationCode()'><span class='txt'>{{ __('인증코드') }}</span></button>
                    ...
                    <div class="i-txt w-full" id='verificatioCode' style='display:none'>
                        <div class="i-txt__wrap" style='display: flex;'>
                            <input type="text" name="code" class="i-txt__input" style='margin-right: 6px;' title="{{ __('please_enter_email') }}" >
                            <!-- <button type="button" class="i-txt__del" title="{{ __('delete_input') }}"><img
                                    src="{{ asset('/public/pub') }}/img/elimination@2x.png"></button> -->
                            <button type='button' id='verificationButton' class='btn-solid large' style='width:18%' onclick='verificationCode()'><span class='txt'>{{ __('인증') }}</span></button>
                        </div>
                    </div>

                    <div class="i-txt password w-full">
                        <label for="pw" class="i-txt__label">{{ __('password') }}</label>
                        <div class="i-txt__wrap">
                            <input type="password" id="pw" name="password" class="i-txt__input" 
                                title="{{ __('password_requirements') }}" placeholder="{{ __('password_requirements') }}"
                                value="{{ old('password') }}">
                            <button type="button" class="i-txt__btn-look" id="i-txt__btn-look-pw"
                                title="{{ __('show_password') }}">
                                <span class="txt blind">{{ __('show_password') }}</span>
                            </button>
                        </div>
                    </div>

                    <div class="i-txt password w-full">
                        <label for="pw2" class="i-txt__label">{{ __('confirm_password') }}</label>
                        <div class="i-txt__wrap">
                            <input type="password" id="pw2" name="password1" class="i-txt__input" 
                                title="{{ __('please_reenter_password') }}"
                                placeholder="{{ __('please_reenter_password') }}"
                                value="{{ old('password1') }}">
                    ...
                    <div class="btn-wrap join-done flex-c-s">
                        <button type="submit" class="btn-solid small">
                            <span class="txt">{{ __('sign_up') }}</span>
                        </button>
                    </div>
                </div>
            </form>
            ...
            <div class="go-reg flex-c-s">
                <p class="info-txt">{{ __('already_a_member') }} <a href="{{ setRoute('user.login') }}"
                        class="point-color tdu">{{ __('login') }}</a></p>
            </div>
        </div>
    </div><!-- //container End -->
...
@push('script')
    <script>
        ...
        function submitCheck(){
            ...
            if(!firstName){
                alert("{{ __('please_enter_first_name') }} ");
                return false;
            } else if(!last_name){
                alert("{{ __('please_enter_last_name') }} ");
                return false;
            } else if(!email){
                alert("{{ __('이메일을 입력 해주세요') }}");
                return false;     
            } else if(verificationCheck == 0){
                alert("{{ __('이메일 인증 해주세요') }} ");
                return false;
            } else if(!pw){
                alert("{{ __('Enter Password') }} ");
                return false;
            } else if(!pw2){
                alert("{{ __('Enter Confirm Password') }} ");
                return false;
            } else if(!validatePassword(pw)){
                alert("{{ __('password_requirements') }} ");
                return false;
            } else if (pw !== pw2){
                alert("{{ __('The password and confirmation password do not match.') }} ");
                return false;
            } 
        }
        ...
        function sendEmail(email){
                $.ajax({
                url:"{{ setRoute('user.register.send.email') }}",
                method: 'POST',
                data: {
                    email: email,
                    _token: '{{ csrf_token() }}' // CSRF 토큰
                },
                success: function(response) {
                    if(response == -1){ // 이메일 중복 일때
                        alert("{{ __('이미 가입된 이메일 입니다') }}");
                        return false;
                    } else {
                        alert("{{ __('Email sent successfully.') }}");
        ...
        function verificationCode(){
            ...
            if(!code){
                alert("{{ __('please Enter The Code') }}");
                return false;
            }

            $.ajax({
                url:"{{ setRoute('user.register.code.verification') }}",
                method: 'POST',
                data: {
                    code:code,
                    email:email,
                ...
                success: function(response) {
                    if(response.message === 1){ // 코드 일치
                        alert("{{ __('Email verified successfully.') }}");
