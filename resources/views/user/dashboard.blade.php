...
@section('content')
    {{-- <div class="noti">
        <a href="#" class="noti-txt">{{ __('Hello! Please check the notice on Anaki!') }}</a>
        <button class="noti-del" onclick="delNoti();" title="알림창 없애기"><img
                src="{{ asset('/public/pub') }}/img/close-12@2x.png"></button>
    </div> --}}

    <div class="content">
        <h2 class="page-tit">{{ __('Dashboard') }}</h2>

        <div class="container-row card-box-wrap mt-0">
            <div class="card-box tether h-132" onclick="window.location.href = '{{ route('user.wallet.index') }}';"
                style="cursor: pointer;">
                <strong class="cb-tit">{{ __('Tether Balance') }}</strong>
                <p class="tether-dt"><strong>$&nbsp{{ number_format($usdtBalance, 2) }}</strong></p>
                <p class="pending">{{ __('Pending') }} : $&nbsp{{ number_format($usdtPending, 2) }}</p>
            </div>

            <div class="card-box ico ico-tip h-132">
                <strong class="cb-tit">{{ __('Ongoing Trade') }}</strong>
                <p class="cb-info">{{ $ongoing_trade > 0 ? $ongoing_trade : __('No ongoing trades') }}</p>
            </div>
        </div>

        <div class="container-row card-box-wrap">
            <div class="card-box ico ico-cs h-150" onclick="window.location.href = '{{ route('user.cs.index') }}';"
                style="cursor: pointer;">
                <strong class="cb-tit">{{ __('Customer Service') }}</strong>
                <p class="cs-cnt"><strong>{{ $active_ticket }}</strong> {{ __('case(s)') }}</p>
            </div>

            <div class="card-box ico ico-mauth h-150" onclick="window.location.href = '{{ route('user.authorize.kyc') }}';"
                style="cursor: pointer;">
                <strong class="cb-tit">{{ __('KYC Verification') }}</strong>
                <p class="mauth-txt {{ $user->kyc_verified != 0 ? 'verified' : '' }}">{{ $user->kyc_verified != 0 ? __('Verified') : __('Unverified') }}</p>
            </div>

            <div class="card-box ico ico-google h-150"
                onclick="window.location.href = '{{ route('user.security.google.2fa') }}';" style="cursor: pointer;">
                <strong class="cb-tit">{{ __('2FA Verification') }}</strong>
                <p class="gg-info {{ $user->two_factor_verified > 0 ? 'verified' : '' }}">
                    {{ $user->two_factor_verified > 0 ? __('2FA is activated') : __('2FA is not activated') }}</p>
                @if ($user->two_factor_verified == 0)
                    <a href="{{ setRoute('user.security.google.2fa') }}"
                        class="gg-auth-link">{{ __('Activate Google 2FA') }}</a>
                @endif
            </div>
        </div>
                    ...
                    <table class="tbl-list dsbo-tbl">
                        ...
                        <tbody id="trade-list-container">
                            @if (isset($listData) && $listData->count() > 0)
                                @include('user.dashboard.partials.trade-list')
                            @endif
                        </tbody>
                    </table>
                </div>
        ...
        <div class="ps-inner">
            <div class="ps-container">
                <div class="ps-head">
                    ...
                    <h2 class="ps-tit" id='sell_title'>{{ __('거래 상세 정보') }}</h2>
                </div>
                        ...
                        <ul class="summary">
                            ...
                            <li>
                                ...
                                <div class="data">
                                    ...
                                    <p class="market-price">{{ __('시장가') }}: 1376 KRW</p>
                                </div>
                            </li>
                            <li>
                            ...
                            <li>
                                <span class="label"id='quantity'>{{ __('테더 판매 수량') }}</span>
                                <div class="data">
    ...
    <div class="pop-sheet" data-pop-sheet="buy_detail">
        <div class="ps-inner">
            <div class="ps-container">
                <div class="ps-head">
                    <h2 class="ps-tit" id='buy_title'>{{ __('거래 상세 정보') }}</h2>
                </div>
                        ...
                        <div class="dt-buy">
                            ...
                            <div class="buy-row">
                                ...
                                <div class="attach-file">
                                    <p class="att-txt">{{ __('※ 지급 증명 첨부 파일은 확장자 JPEG, JPG, PNG, GIF 파일만 가능합니다.') }}</p>
...
@push('script')
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const encrypt_order_id = '{{ urlSafeEncrypt($order_id) }}';
        ...
        function loadMoreItems() {
            currentPage++;
            fetch(`{{ route('user.dashboard') }}?page=${currentPage}`, {
                    headers: {
        ...
        function showTradeDetails(_orderId, encryptOrderId, openchat) {
            ...
            fetch(`{{ route('user.mytrade.details', '') }}/${encryptOrderId}`)
                ...
                .then(data => {
                    ...
                    var newChatSign = document.getElementById(chat_id+'-'+{{ Auth::user()->id }});
                    if (newChatSign) {
                        startChatAfterDelay();
                        newChatSign.remove();
                    }
        ...
        function updateSellDetailPopup(data) {
           ...
           if(data.order_type === 'buy'){
                if(data.client_email === "{{ auth()->user()->email }}" ){
                    sellTitle.textContent = "{{ __('판매하기-판매자 거래정보') }}";
                } else {
                    sellTitle.textContent = "{{ __('판매하기-구매자 거래정보') }}";
                }
            } else {
                if(data.client_email === "{{ auth()->user()->email }}" ){
                    sellTitle.textContent = "{{ __('구매하기-구매자 거래정보') }}";
                } else {
                    sellTitle.textContent = "{{ __('구매하기-판매자 거래정보') }}";
                }
            }
        
            if (data.state === "open" || data.state === 'send') {
                statusElement.textContent = "{{ __('진행중') }}";
                statusElement.className = "data-txt going";
            } else if (data.state === "dispute") {
                statusElement.textContent = "{{ __('분쟁중') }}";
                statusElement.className = "data-txt going";
            } else if (data.state === "done") {
                statusElement.textContent = "{{ __('완료') }}";
                statusElement.className = "data-txt completed";
            } else if (data.state === "cancel") {
                statusElement.textContent = "{{ __('취소') }}";
            ...
            document.querySelector('[data-pop-sheet="sell_detail"] .market-price').textContent =
                `{{ __('시장가') }}: ${parseFloat(data.market_price).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')} {{ __('KRW') }}`;
            ...
            document.querySelector('[data-pop-sheet="sell_detail"] li:nth-child(5) .data-txt').textContent =
                `${parseFloat(data.total_amount).toFixed(2).replace(/\d(?=(\d{3})+\.)/g, '$&,')} {{ __('KRW') }}`;
            ...
            if (data.isTransferred) {
                ...
                transferButton.querySelector('.txt').textContent = "{{ __('테더 전송 완료') }}";
            } else {
                transferButton.disabled = false;
                transferButton.style.opacity = '1';
                transferButton.style.cursor = 'pointer';
                transferButton.querySelector('.txt').textContent = "{{ __('테더 이체') }}";
            }
        ...
        //                         clearInterval(checkInterval);
        //                         alert(
        //                             "{{ __('No matching deposit found after 120 seconds. Please try again or contact support.') }}"
        //                         );
        //                     }
        //                 }
        //             })
        ...
        //                 console.error('Error:', error);
        //                 clearInterval(checkInterval);
        //                 alert("{{ __('An error occurred while checking for deposits.') }} " + error);
        //             });
        //     }
        ...
        function updateBuyDetailPopup(data) {
            ...
            if(data.order_type === 'sell'){
                if(data.client_email === "{{ auth()->user()->email }}" ){
                    buyTitle.textContent = "{{ __('구매하기-구매자 거래정보') }}";
                } else {
                    buyTitle.textContent = "{{ __('구매하기-판매자 거래정보') }}";
                }
            } else {
                if(data.client_email === "{{ auth()->user()->email }}" ){
                    buyTitle.textContent = "{{ __('판매하기-판매자 거래정보') }}";
                } else {
                    buyTitle.textContent = "{{ __('판매하기-구매자 거래정보') }}";
                }
            }
            // Update status
            ...
            document.querySelector('[data-pop-sheet="buy_detail"] li:nth-child(5) .data-txt').textContent =
                `${parseFloat(data.total_amount).toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ",")} {{ __('KRW') }}`;
            ...
            if (data.pay_proof) {
                attachImgDiv.innerHTML = `<img src="/frontend/proof/${data.pay_proof}" alt="{{ __('지급 증빙 이미지') }}">`;
