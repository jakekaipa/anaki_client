...
Route::name('user.')->group(function(){
    ...
    Route::post('languageChange',[UserLoginController::class,"languageChange"])->name('languageChange');
    Route::get('register/{referralCode?}',[UserRegisterController::class,"showRegistrationForm"])->name('register');
    Route::post('register',[UserRegisterController::class,"register"])->name('register.submit');
    // 이메일 인증 코드 보내기
    Route::post('sendEmailVerificationCode',[UserRegisterController::class,"sendEmailVerificationCode"])->name('register.send.email');
    // 이메일 인증
    Route::post('codeVerification',[UserRegisterController::class,"codeVerification"])->name('register.code.verification');
    ...
    Route::controller(AuthorizationController::class)->prefix("authorize")->name('authorize.')->middleware("auth")->group(function(){
        ...
        Route::get('sms/resend/{token}', 'smsResendToken')->name('sms.resend');
