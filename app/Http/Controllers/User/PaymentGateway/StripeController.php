    ...
    public function stripePayment(Request $request)
    {
        ...
        if (isset($payment->id)) {
            ...
            try {
                ...
                #Saved transaction
                DB::commit();
                return redirect('user/money/add')->with(['success' => ['Transaction completed successfully']]);
            } catch (Exception $e) {
                info($e);
                DB::rollback();
                return redirect('user/money/add')->with(['error' => ['An error occurred. Please try again.']]);
            }
        }
    }
}
