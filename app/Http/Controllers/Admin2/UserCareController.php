...
class UserCareController extends Controller
{
    ...
    public function userDetails($username)
    {
        ...
        if (!$user) return back()->with(['error' => ['User does not exist.']]);
        ...
        $usdtBalance = "" . $usdtBalance . "(pending:" . $usdtPending . ")";
        $trxBalance = "0";

        try {
            $wallet = new \App\Models\Mongo\Wallet();

            $userWallet = $wallet->where('address', $user->walletAddress)->first();
            if ($userWallet) {
                $trxBalance = $user->trxBalance;
            }
        } catch (\Exception $e) {
            Log::info("WalletController.getBalance error:" . $e->getMessage());
        }
    ...
    public function kycReject(Request $request, $username)
    {
        ...
        return back()->with(['success' => ['User KYC information rejected.']]);
    }
    ...
    public function walletBalanceUpdate(Request $request, $username)
    {
        ...
        try {
            ...
            $notification_content = [
                ...
                'message' => __('Your Wallet (:currency) balance has been updated', ['currency' => $user_wallet->currency->code]),
            ...
            DB::rollBack();
            return back()->with(['error' => ['Transaction failed: (error message).']]);
        }
