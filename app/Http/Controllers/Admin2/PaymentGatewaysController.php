...
class PaymentGatewaysController extends Controller
{
    ...
    public function registerSlugTypes($type) {
        $slug_types = [
            'view' => [
                ...
                'receiving-method' => [
                    'manual'    => 'manualReceivingMethodView',
                ],
            ],
            'edit'  => [
                'add-money' => [
                ...
                'receiving-method' => [
                    'manual'    => 'manualReceivingMethodEdit',
                ],
            ],
            'update'    => [
                'add-money' => [
                ...
                'receiving-method' => [
                    'manual'    => 'manualReceivingMethodUpdate',
                ],
            ],
            'store'     => [
                'add-money' => [
                ...
                'receiving-method' => [
                    'manual'    => 'manualReceivingMethodStore',
                ],
            ],
            'create'    => [
                'add-money'   => [
                ...
                'receiving-method' => [
                    'manual'    => 'manualReceivingMethodCreate',
                ],
            ],
        ];
     ...
     * @return view
     */
    public function manualReceivingMethodView() {
        $page_title = "Manual Receiving Method";
        $payment_gateways = PaymentGateway::receivingMethod()->manual()->get();
        return view('admin.sections.payment-gateways.receiving-method.manual.index',compact(
            'page_title',
            'payment_gateways',
        ));
    }
     ...
     * @return view
     */
    public function manualReceivingMethodEdit($alias) {
        $page_title = "Manual Receiving Method Edit";
        $currencies = Currency::orderByDesc('default')->active()->get();
        $payment_gateway = PaymentGateway::receivingMethod()->manual()->gateway($alias)->firstOrFail();
        return view('admin.sections.payment-gateways.receiving-method.manual.edit',compact(
            'page_title',
            'payment_gateway',
            'currencies',
        ));
     ...
     * Function for create new Manual Add Money Gateway
     */
    public function manualReceivingMethodCreate() {
        $page_title = "Receiving Method Create";
        $currencies = Currency::orderByDesc('default')->active()->get();
        return view('admin.sections.payment-gateways.receiving-method.manual.create',compact(
            'page_title',
            'currencies'
        ));
    }
     ...
     * @return view
     */
    public function manualReceivingMethodStore(Request $request) {

        $gateway_name = $request->gateway_name;
        $validator = Validator::make($request->all(),[
            'gateway_name'          => ['required','string','max:60',Rule::unique('payment_gateways','alias')->where(function($query) use ($gateway_name) {
                $alias = Str::slug($gateway_name);
                $query->where('slug',PaymentGatewayConst::receiving_method_slug())->where('type',PaymentGatewayConst::MANUAL)->where('alias',$alias);
            })],
        ...
        $validator->after(function ($validator) use ($gateway_name) {
            // Search Gateway is unique or not
            if(PaymentGateway::receivingMethod()->manual()->gateway(Str::slug($gateway_name))->exists()) {
                $validator->errors()->add(
                    'gateway_name', 'The gateway name has already been taken.'
                );
            }
        ...
        $validated['alias']                 = Str::slug($validated['gateway_name']);
        $validated['name']                  = $validated['gateway_name'];
        $validated['slug']                  = Str::slug(PaymentGatewayConst::RECEIVING_METHOD);
        $validated['title']                 = $validated['name'] . " " . "Gateway";
        $validated['type']                  = PaymentGatewayConst::MANUAL;
        $validated['last_edit_by']          = Auth::user()->id;
        $validated['supported_currencies']  = [$validated['currency_code']];
     ...
     * @param string $alias
     */
    public function manualReceivingMethodUpdate(Request $request,$alias) {
        // Find gateway is available or not
        $gateway = PaymentGateway::receivingMethod()->manual()->gateway($alias)->first();
        if(!$gateway) {
            return back()->with(['error' => ['Payment gateway not found.']]);
        }
        ...
        $validator = Validator::make($request->all(),[
            'gateway_name'          => ['required','string','max:60',Rule::unique('payment_gateways','alias')->where(function($query) use ($gateway_name, $gateway) {
                $alias = Str::slug($gateway_name);
                $query->whereNot('id',$gateway->id)->where('slug',PaymentGatewayConst::receiving_method_slug())->where('type',PaymentGatewayConst::MANUAL)->where('alias',$alias);
            })],
            'desc'                  => 'nullable|string|max:10000',
            'label'                 => 'nullable|array',
            'label.*'               => 'nullable|string|max:50',
        ...
        $validator->after(function ($validator) use ($gateway_name,$gateway) {
            ...
            if(PaymentGateway::whereNot(function($query) use ($gateway) {
                ...
                $alias = Str::slug($gateway_name);
                $query->where('slug',PaymentGatewayConst::receiving_method_slug())
                ->where('type',PaymentGatewayConst::MANUAL)
                ->where('alias',$alias);
            })->exists()) {
                $validator->errors()->add(
        ...
        return redirect()->route('admin.payment.gateway.view',['receiving-method','manual'])->with(['success' => ['Payment gateway updated successfully.']]);
    }
